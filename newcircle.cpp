...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>            
    if(angle>=0 && angle<=90)
        {   
            angle*=M_PI/180.;
            double perpendicular=radius*sin(angle);                   //perpendicular.=hypot*sin(theeta)
            double baseTriangle=sqrt(radius*radius-perpendicular*perpendicular);
            int baseX1=(radius-baseTriangle)+.5;
            for(int column=graphWidth/2+radius,CX=radius;column>=graphWidth/2-radius,CX>=radius-baseX1;column--,CX--)
                {
                    double CY=(sqrt(radius*radius-CX*CX))/2.0;  //equation of circle (pyth. theorm)
    ...
    else if(angle>90&&angle<=180)
            {
                angle=angle-90;
                angle*=M_PI/180.;
                double perpendicular1=radius*sin(angle);                   //perpendicular.=hypot*sin(theeta)
                int perpendicular=perpendicular1+.5;
            
                    for(int column=graphWidth/2+radius,CX=radius;column>=graphWidth/2-perpendicular,CX>=-perpendicular;column--,CX--)
                        {
                            double CY=(sqrt(radius*radius-CX*CX))/2.0;  //equation of circle (pyth. theorm)
                            int upperarc=graphHeight/2-CY+.5;
                            circle[upperarc][column]='.';
        ...
        { 
            ...
            ///////////////upper curve/////////////////////////////////////////////////////////////
            for(int column=graphWidth/2+radius,CX=radius;column>=graphWidth/2-radius,CX>=-radius;column--,CX--)
                {
                    double CY=(sqrt(radius*radius-CX*CX))/2.0;  //equation of circle (pyth. theorm)
                    int upperarc=graphHeight/2-CY+.5;
                    circle[upperarc][column]='.';
            ...
            for(int column=graphWidth/2-radius , CX=-radius; column<=(graphWidth/2-radius)+base,CX<=-baseTriangle;CX++,column++)
                {
